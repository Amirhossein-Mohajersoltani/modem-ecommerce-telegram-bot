...
async def add_admin_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if database.is_owner(update.effective_chat.id):
        ...
        await context.bot.send_message(
            ...
            text="لطفا آیدی ادمین جدید را ارسال کنید:",
            reply_markup=reply_markup
        )
        return ADD_ADMIN_BY_ID
...
async def add_admin_by_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("add_product") or context.user_data.get("delete_admin") or context.user_data.get(
        ...
        'update_user_info_conv') or context.user_data.get(
    ...
    if not database.is_contain(admin_id,"users",f" id = {admin_id}"):
    ...
    if database.is_contain(admin_id,"users",f"id = {admin_id}"):

        # Check if the process done successfully or not
        if database.update_record(Record(table_name="users", is_admin=True),condition=f"id = {admin_id}"):
            text="عملیات با موفقیت انجام شد."
            result = database.fetch_data("users", f"id = {admin_id}")
            new_admin_chat_id = result[0][1]
            admin_reply_markup = create_admin_panel()
            if not database.is_owner(new_admin_chat_id):
                await context.bot.send_message(chat_id=new_admin_chat_id, text="شما به ادمین ارتقا یافتید.", reply_markup=admin_reply_markup)
        else:
...
def add_admin_handler():
    add_admin_conv_handler = ConversationHandler(
        entry_points=[MessageHandler(filters.Regex("^اضافه کردن ادمین$"), add_admin_button)],
        states={
            ADD_ADMIN_BY_ID: [
                MessageHandler(filters.Regex("^اضافه کردن ادمین$"), add_admin_button),
                MessageHandler(filters.Regex("^لغو عملیات!$"), admin_owner_panel_break_conversation),
                MessageHandler(filters.TEXT & (~filters.COMMAND), add_admin_by_id)
            ],
        },
        fallbacks=[CommandHandler('done', owner_panel_break_conversation)],
    )
    return add_admin_conv_handler