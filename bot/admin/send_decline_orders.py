...
async def send_declined_orders(update, context: ContextTypes.DEFAULT_TYPE):
    if database.is_admin(update.effective_user.id) or database.is_owner(update.effective_user.id):
        data = database.fetch_data("declined_orders")

        if not data:
            await update.message.reply_text("هیچ سفارش رد ‌شده‌ای یافت نشد.")
            return
        ...
        filename_csv = "declined_orders.csv"
